...
-- Also: login_player must call extensions.gen_random_bytes
-- because pgcrypto lives in the 'extensions' schema, not 'public'.
-- Without the schema prefix, SET search_path=public hides it and
-- the function errors → client falls back to fake _fb_ tokens that
-- are never written to DB → all session-validated RPCs reject them.
-- =============================================================
